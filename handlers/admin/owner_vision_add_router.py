from aiogram import Router, F, Bot
from aiogram.types import Message, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton
from aiogram.fsm.context import FSMContext
from aiogram.exceptions import TelegramBadRequest

from sqlalchemy import select

from database.models import Person, Vision
from database.session import AsyncSessionLocal
from config import OWNER_IDS
from forms.forms_fsm import OwnerClientsStates, OwnerVisionAddStates
from datetime import date

owner_vision_add_router = Router()

def is_owner(user_id: int) -> bool:
    return user_id in OWNER_IDS

# Начало добавления записи зрения
@owner_vision_add_router.callback_query(OwnerClientsStates.viewing_client_profile, F.data.startswith("add_vision_"))
async def start_add_vision(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if not is_owner(callback.from_user.id):
        await callback.answer("Доступ запрещён", show_alert=True)
        return

    person_id = int(callback.data.split("_")[2])
    await state.update_data(person_id=person_id)

    try:
        await callback.message.delete()
    except TelegramBadRequest:
        pass

    await bot.send_message(
        callback.from_user.id,
        "➕ <b>Добавление новой записи зрения</b>\n\n"
        "<b>Шаг 1/3: Параметры зрения</b>\n\n"
        "Введите 6 значений через пробел:\n"
        "SPH R   CYL R   AXIS R\n"
        "SPH L   CYL L   AXIS L\n\n"
        "Пример:\n"
        "-1.50 -0.50 180 -2.00 -1.00 90\n\n"
        "Если какого-то значения нет — ставьте 0 или пропустите (но лучше указать).",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="◀ Отмена", callback_data="cancel_add_vision")]
        ])
    )
    await state.set_state(OwnerVisionAddStates.waiting_sph_cyl_axis)
    await callback.answer()

# Отмена на любом шаге
@owner_vision_add_router.callback_query(F.data == "cancel_add_vision")
async def cancel_add_vision(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if not is_owner(callback.from_user.id):
        return

    try:
        await callback.message.delete()
    except TelegramBadRequest:
        pass

    data = await state.get_data()
    person_id = data.get("person_id")

    if person_id:
        async with AsyncSessionLocal() as session:
            person = await session.get(Person, person_id)
        if person:
            from handlers.owner.crud.clients_router import show_client_profile  # импорт функции профиля
            await show_client_profile(callback, person, state, bot)

    await state.clear()
    await callback.answer("Добавление отменено")

# Шаг 1: SPH, CYL, AXIS
@owner_vision_add_router.message(OwnerVisionAddStates.waiting_sph_cyl_axis)
async def process_sph_cyl_axis(message: Message, state: FSMContext, bot: Bot):
    if not is_owner(message.from_user.id):
        return

    values = message.text.strip().split()
    if len(values) != 6:
        await message.answer(
            "❌ Нужно ровно 6 значений (SPH R, CYL R, AXIS R, SPH L, CYL L, AXIS L).\n"
            "Повторите или отмените.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="◀ Отмена", callback_data="cancel_add_vision")]
            ])
        )
        return

    try:
        sph_r = float(values[0])
        cyl_r = float(values[1])
        axis_r = int(float(values[2]))  # AXIS обычно целое
        sph_l = float(values[3])
        cyl_l = float(values[4])
        axis_l = int(float(values[5]))
    except ValueError:
        await message.answer("❌ Все значения должны быть числами. Повторите.")
        return

    await state.update_data(
        sph_r=sph_r, cyl_r=cyl_r, axis_r=axis_r,
        sph_l=sph_l, cyl_l=cyl_l, axis_l=axis_l
    )

    await message.answer(
        "<b>Шаг 2/3: PD, тип линз, модель оправы</b>\n\n"
        "Введите через пробел:\n"
        "PD   lens_type   frame_model\n\n"
        "Пример:\n"
        "62 progressive Ray-Ban RB2132\n\n"
        "PD — обязательно число.\n"
        "Тип линз и модель оправы — текст (можно пропустить, написав только PD).",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="◀ Отмена", callback_data="cancel_add_vision")]
        ])
    )
    await state.set_state(OwnerVisionAddStates.waiting_pd_lens_frame)

# Шаг 2: PD, lens_type, frame_model
@owner_vision_add_router.message(OwnerVisionAddStates.waiting_pd_lens_frame)
async def process_pd_lens_frame(message: Message, state: FSMContext, bot: Bot):
    if not is_owner(message.from_user.id):
        return

    parts = message.text.strip().split(maxsplit=2)

    if not parts:
        await message.answer("❌ Укажите хотя бы PD. Повторите.")
        return

    try:
        pd = float(parts[0])
    except ValueError:
        await message.answer("❌ PD должен быть числом. Повторите.")
        return

    lens_type = parts[1] if len(parts) >= 2 else None
    frame_model = parts[2] if len(parts) >= 3 else None

    await state.update_data(pd=pd, lens_type=lens_type, frame_model=frame_model)

    await message.answer(
        "<b>Шаг 3/3: Примечание</b>\n\n"
        "Введите примечание (опционально). Можно просто отправить пустое сообщение.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="◀ Отмена", callback_data="cancel_add_vision")]
        ])
    )
    await state.set_state(OwnerVisionAddStates.waiting_note)

# Шаг 3: Note и сохранение
@owner_vision_add_router.message(OwnerVisionAddStates.waiting_note)
async def process_note_and_save(message: Message, state: FSMContext, bot: Bot):
    if not is_owner(message.from_user.id):
        return

    note = message.text.strip() if message.text else None

    data = await state.get_data()
    person_id = data["person_id"]

    async with AsyncSessionLocal() as session:
        person = await session.get(Person, person_id)
        if not person:
            await message.answer("❌ Клиент не найден.")
            await state.clear()
            return

        new_vision = Vision(
            person_id=person_id,
            visit_date=date.today(),
            sph_r=data.get("sph_r"),
            cyl_r=data.get("cyl_r"),
            axis_r=data.get("axis_r"),
            sph_l=data.get("sph_l"),
            cyl_l=data.get("cyl_l"),
            axis_l=data.get("axis_l"),
            pd=data.get("pd"),
            lens_type=data.get("lens_type"),
            frame_model=data.get("frame_model"),
            note=note
        )
        session.add(new_vision)

        # Обновляем дату последнего визита
        person.last_visit_date = date.today()

        await session.commit()

    await message.answer("✅ Новая запись зрения успешно добавлена!")

    # Возврат в профиль
    from handlers.owner.crud.clients_router import show_client_profile
    await show_client_profile(message, person, state, bot)
    await state.set_state(OwnerClientsStates.viewing_client_profile)