...
@owner_content_router.message(Command("owner"))
...
# ... остальной код роутера без изменений ...
...
async def process_edit_or_cancel(message: Message, state: FSMContext):
    ...
    # Если это не кнопка выхода — считаем новым текстом
...
# Полный выход из панели (если нажать кнопку в меню выбора разделов)
@owner_content_router.message(OwnerContentStates.choosing_section, F.text == "◀ Выйти из панели")
async def exit_from_choosing(message: Message, state: FSMContext):
    if not is_owner(message.from_user.id):
        return
...
# Полный выход из панели (работает из любого состояния)
